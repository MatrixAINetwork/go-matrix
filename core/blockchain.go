...
func NewBlockChain(db mandb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	...
	matrixRoots, _ := lru.New(matrixRootLimit)
	...
	bc := &BlockChain{
		...
		matrixCache:     state.NewDatabase(db),
		quit:            make(chan struct{}),
		bodyCache:       bodyCache,
		bodyRLPCache:    bodyRLPCache,
		blockCache:      blockCache,
		futureBlocks:    futureBlocks,
		matrixRootCache: matrixRoots,
		engine:          engine,
		vmConfig:        vmConfig,
		badBlocks:       badBlocks,
	}
...
func (bc *BlockChain) insert(block *types.Block) {
	...
	if common.IsBroadcastNumber(block.NumberU64()) {
		//state ,err := bc.State()
		//if err == nil{
		//	SetBroadcastTxs(state,block, bc.chainConfig.ChainId)
		//}
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, matrixState *state.StateDB) (status WriteStatus, err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
	...
	matrixRoot, err := matrixState.Commit(bc.chainConfig.IsEIP158(block.Number()))
	if err != nil || (matrixRoot == common.Hash{}) {
		return NonStatTy, err
	}
	...
	rawdb.WriteMatrixRoot(bc.db, block.Hash(), block.NumberU64(), matrixRoot)
