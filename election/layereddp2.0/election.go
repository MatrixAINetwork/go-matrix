...
func init() {
	baseinterface.RegElectPlug(manparams.ElectPlug_layerdDPV2, RegInit)
}
...
func (self *layeredDdp) upDateEleDpi(dpElect *support.ElectDP, eleDpi *mc.ElectDynamicPollingInfo) {
	eleDpi.CandidateList = dpElect.GenUsableCandidateList()
	eleDpi.MinerNum = calcMinerNum(uint64(len(eleDpi.CandidateList)), dpElect.EleCfg.MinerNum)
	eleDpi.Seq++
	eleDpi.Number = dpElect.Num
	log.Trace("动态选举方案", "生成下一轮数据", eleDpi.Seq, "矿工选举数目", eleDpi.MinerNum, "高度", eleDpi.Number, "候选节点个数", len(eleDpi.CandidateList))
...
func (self *layeredDdp) minerdp(eleDpi *mc.ElectDynamicPollingInfo, dpElect *support.ElectDP) (choosedNodes []common.Address) {
	...
	usableNodeList := dpElect.GetUsableNodeListV2(eleDpi.CandidateList, nil)
	...
	//更新候选列表
	usableNodeList = dpElect.GetUsableNodeListV2(nil, choosedNodes) //使用第二轮的节点，移除已选出的节点, 生成初选列表
