    11.15：zhenghe pushed to branch master_1115 at repository go-matrix
...
delete old address	(Author: zhangwen)
fmt	(Author: zhangwen)
Update sync.go for fastSyncMode	(Author: zhangwen)
web3去掉调试打印	(Author: zhangwen)
miner跑交易增加委托gas判断，防止state不一致	(Author: zhangwen)
miner跑交易增加委托gas判断，防止state不一致	(Author: zhangwen)
miner跑交易增加委托gas判断，防止state不一致	(Author: zhangwen)
miner跑交易增加委托gas判断，防止state不一致	(Author: zhangwen)
miner跑交易增加委托gas判断，防止state不一致	(Author: zhangwen)
del self	(Author: zhangwen)
add reset	(Author: zhangwen)
fmt	(Author: zhangwen)
add log	(Author: zhangwen)
modify	(Author: zhangwen)
Update deposit.go	(Author: zhangwen)
modify deposit	(Author: zhangwen)
Merge branch 'master' of 192.168.3.81:matrix/go-matrix	(Author: zhangwen)
增加setVoted方法，标识节点已对某共识请求投过票,在相关投票操作完成之后调用该方法标识该共识请求投票完成	(Author: zhangwen)
修复拓扑图更新时，上位节点在最后时，无法更新拓扑图的bug	(Author: zhangwen)
Merge branch 'master' into master_nodeId	(Author: zhangwen)
Update .gitlab-ci.yml	(Author: zhangwen)
Update .gitlab-ci.yml	(Author: zhangwen)
Update .gitlab-ci.yml	(Author: zhangwen)
Merge branch 'master' of gitlab.matrix.com:matrix/go-matrix	(Author: zhangwen)
mac building update by laolv	(Author: zhangwen)
Update .gitlab-ci.yml	(Author: zhangwen)
Update .gitlab-ci.yml	(Author: zhangwen)
Update .gitlab-ci.yml	(Author: zhangwen)
Update .gitlab-ci.yml	(Author: zhangwen)
...
Update .gitlab-ci.yml	(Author: zhangwen)
Update .gitlab-ci.yml	(Author: zhangwen)
windows building update by shang shun	(Author: zhangwen)
默认创世文件补全	(Author: zhangwen)
Merge branch 'master' into master_nodeId	(Author: zhangwen)
修改查询抵押web3返回man地址	(Author: zhangwen)
默认配置文件被覆盖，重新提交	(Author: zhangwen)
Merge branch 'master' into master_nodeId	(Author: zhangwen)
Merge branch 'master_nodeId'	(Author: zhangwen)
modify signature file	(Author: zhangwen)
去掉高层使用fetch请求区块 add version	(Author: zhangwen)
区块同步fech增加log打印	(Author: zhangwen)
修改默认创世文件配置	(Author: zhangwen)
修改默认创世文件配置	(Author: zhangwen)
修改默认创世文件配置	(Author: zhangwen)
区块同步fetch增加log打印等	(Author: zhangwen)