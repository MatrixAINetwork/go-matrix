...
func (self *masterCache) SaveInquiryReq(req *mc.HD_V2_ReelectInquiryReqMsg) common.Hash {
	if nil == req {
...
func (self *masterCache) IsMatchedInquiryRsp(rsp *mc.HD_V2_ReelectInquiryRspMsg) error {
	if nil == rsp {
...
func (self *masterCache) GenRLReqMsg(inquiryAgreeVotes []common.Signature) error {
	...
	inquiryMsg, OK := self.inquiryPool.getReqMsg().(*mc.HD_V2_ReelectInquiryReqMsg)
	if OK == false || inquiryMsg == nil {
		return errors.New("获取询问消息失败")
	}

	reqMsg := &mc.HD_V2_ReelectLeaderReqMsg{
...
func (self *masterCache) GenBroadcastMsgWithInquiryResult(result mc.ReelectRSPType, rsp *mc.HD_V2_ReelectInquiryRspMsg) error {
	...
	broadcastMsg := &mc.HD_V2_ReelectBroadcastMsg{
		Number:    self.number,
		Type:      result,
...
func (self *masterCache) GenBroadcastMsgWithRLSuccess(rlAgreeVotes []common.Signature) error {
	...
	rlReq, OK := self.rlReqPool.getReqMsg().(*mc.HD_V2_ReelectLeaderReqMsg)
	...
	rlResult := &mc.HD_V2_ReelectLeaderConsensus{
		Req:   rlReq,
		Votes: rlAgreeVotes,
	}
	broadcastMsg := &mc.HD_V2_ReelectBroadcastMsg{
...
func (self *masterCache) GetRLReqMsg() (*mc.HD_V2_ReelectLeaderReqMsg, common.Hash, error) {
	if self.inquiryResult != mc.ReelectRSPTypeAgree {
		return nil, common.Hash{}, errors.Errorf("当前询问结果(%v) != ReelectRSPTypeAgree", self.inquiryResult)
	}

	reqMsg, OK := self.rlReqPool.getReqMsg().(*mc.HD_V2_ReelectLeaderReqMsg)
...
func (self *masterCache) GetBroadcastMsg() (*mc.HD_V2_ReelectBroadcastMsg, common.Hash, error) {
	broadcastMsg, OK := self.broadcastPool.getReqMsg().(*mc.HD_V2_ReelectBroadcastMsg)
...
func (self *masterCache) GetLocalBroadcastMsg() (*mc.HD_V2_ReelectBroadcastMsg, error) {
	broadcastMsg, OK := self.broadcastPool.getReqMsg().(*mc.HD_V2_ReelectBroadcastMsg)
	if OK == false || broadcastMsg == nil {
