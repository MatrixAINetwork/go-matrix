...
func New(chain util.ChainReader, st util.StateDB, preSt util.StateDB, ppreSt util.StateDB, coinbase common.Address) reward.Reward {
	...
	if data >= util.CalcEpsilon {
		RC, err := matrixstate.GetAIBlkRewardCfg(preSt)
		if nil != err || nil == RC {
			log.Error("固定区块奖励", "获取状态树配置错误", err)
			return nil
		}
		SetReward := cfg.EpsilionSetRewardNew(preMiner, innerMinerAccounts, coinbase, util.BlkReward, RC)
		rewardCfg := cfg.EpsilionNew(RC, SetReward, preMiner, innerMinerAccounts, util.BlkReward, data)

		return rewardexec.AIBlockNew(chain, rewardCfg, st, interval, foundationAccount, currentTop, originElectNodes)
	} else {
