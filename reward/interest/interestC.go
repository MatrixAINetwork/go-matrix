...
type interestEpsilon struct {
	interestConfig *mc.InterestCfg
	depositCfg     depositcfg.DepositCfgInterface
	bcInterval     *mc.BCIntervalInfo
}

func EpsilonNew(st util.StateDB, preSt util.StateDB, depositCfgVersion string) *interestEpsilon {
	...
	return &interestEpsilon{interestConfig: IC, depositCfg: depositcfg.GetDepositCfg(depositCfgVersion), bcInterval: bcInterval}
}

func (ic *interestEpsilon) calcWeightDeposit(deposit *big.Int, blockInterest uint64) *big.Int {
...
func (ic *interestEpsilon) PayInterest(state vm.StateDBManager, num uint64, time uint64) map[common.Address]*big.Int {
...
func (ic *interestEpsilon) payAllInterest(num uint64, state vm.StateDBManager, time uint64) error {
...
func (ic *interestEpsilon) canPayInterest(state vm.StateDBManager, num uint64, payInterestPeriod uint64) bool {
...
func (ic *interestEpsilon) getLastInterestNumber(number uint64, InterestInterval uint64) uint64 {
...
func (ic *interestEpsilon) GetReward(blockReward *big.Int, depositNodes []common.DepositBase) map[common.Address][]common.OperationalInterestSlash {
...
func (ic *interestEpsilon) CalcInterestReward(totalWeightDeposit, reward *big.Int, weightDepositMap map[common.Address][]common.OperationalInterestSlash) map[common.Address][]common.OperationalInterestSlash {
	...
	broadInterval := ic.bcInterval.GetBroadcastInterval()
	...
	for k, node := range weightDepositMap {
		...
		for _, v := range node {
			...
			blockAmount.Mul(blockAmount, new(big.Int).SetUint64(broadInterval-1))
...
func (ic *interestEpsilon) canCalc(currentState vm.StateDBManager) bool {
	latestNum, err := matrixstate.GetInterestCalcNum(currentState)
	if nil != err {
		log.Error(PackageName, "状态树获取前一发放惩罚高度错误", err)
		return false
	}
	if latestNum > ic.bcInterval.GetLastBroadcastNumber() {
...
func (ic *interestEpsilon) CalcReward(state vm.StateDBManager, num uint64, parentHash common.Hash) {
	//选举周期的开始分配
	if !ic.canCalc(state) {
		return
	}
	if err := matrixstate.SetInterestCalcNum(state, num); err != nil {
...
func (ic *interestEpsilon) SetReward(InterestMap map[common.Address][]common.OperationalInterestSlash, state vm.StateDBManager) {
...
func (ic *interestEpsilon) GetDeposit(parentHash common.Hash) []common.DepositBase {
...
func (ic *interestEpsilon) GetWeightDeposit(depositNodes []common.DepositBase) (map[common.Address][]common.OperationalInterestSlash, *big.Int) {
...
func (ic *interestEpsilon) canCalcInterest(state vm.StateDBManager, num uint64, calcInterestInterval uint64) bool {
